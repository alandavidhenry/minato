...
- **Page furniture**: `src/components/page-header.tsx` (`{title, description?, actions?}` — the one page-title treatment, `text-xl font-medium`, used across `admin/**` and `customer/**`; `customer/welcome-header.tsx` wraps it), `src/components/empty-state.tsx` and `src/components/table-skeleton.tsx` (built on the new `ui/skeleton.tsx`).
