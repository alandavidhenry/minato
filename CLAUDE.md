...
GOTENBERG_BASIC_AUTH_USERNAME=  # only set in deployed environments (Terraform-provisioned, public ACI is locked down with basic auth)
...
**Gotenberg** (`infrastructure/modules/gotenberg/`) — a public Azure Container Instance running `gotenberg/gotenberg:8`, used to convert uploaded Word documents to PDF (`src/lib/document-conversion.ts`). It is *not* VNet-isolated: the App Service Plan is on the Free (F1) SKU, which doesn't support regional VNet integration, so the ACI is reachable over the public internet and locked down with Gotenberg's built-in basic auth instead (credentials generated by Terraform, stored in Key Vault, injected as `GOTENBERG_BASIC_AUTH_USERNAME`/`PASSWORD`). Revisit with private VNet integration if the App Service Plan is ever upgraded to Basic (B1) or above.
