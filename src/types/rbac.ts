...
export enum UserRole {
  ...
  export function hasRole(user: RBACUser | undefined | null, role: UserRole): boolean {
    if (!user) return false;
    return user.roles.includes(role);
  }
  
  export function hasPermission(user: RBACUser | undefined | null, permission: Permission): boolean {
    if (!user) return false;
