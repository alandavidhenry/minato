...
export function ActivityDashboard({ logs }: ActivityDashboardProps) {
  ...
  useEffect(() => {
    ...
    function processActivityData() {
      ...
      const dailyGroups = logs.reduce((acc, log) => {
      ...
      const typeGroups = logs.reduce((acc, log) => {
        if (!acc[log.activityType]) {
          acc[log.activityType] = { name: formatActivityType(log.activityType), value: 0 }
        }
      ...
      setDailyActivity(Object.values(dailyGroups).sort((a, b) => a.date.localeCompare(b.date)))
              ...
              <PieChart>
                ...
                >
                  {activityByType.map((entry) => (
                    <Cell 
                      key={`cell-${entry.name}`} 
                      fill={COLORS[activityByType.findIndex(item => item.name === entry.name) % COLORS.length]} 
                    />
                  ))}
                </Pie>
