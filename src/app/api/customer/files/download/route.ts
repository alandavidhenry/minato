// src/app/api/customer/files/download/route.ts
...
export async function GET(request: NextRequest) {
  ...
  if (!cleanSubpath || cleanSubpath.includes('..')) {
    return NextResponse.json({ error: 'Invalid path.' }, { status: 400 })
  }

  const fullPath = `${company.folderPath}/${cleanSubpath}`

  try {
    const fileManager = getFileManager()
    const url = await fileManager.generateDownloadUrl(fullPath)
    ...
    console.error('Error generating customer file download URL:', error)
    return NextResponse.json(
      { error: 'Failed to generate download URL.' },
      { status: 500 }
    )
