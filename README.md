...
|---|---|---|
| `src/lib/__tests__/` | Unit | `user-database`, `password-reset`, `activity-logger`, `storage`, `url-shortener`, `version-manager`, `list-blobs`, `utils`, `completion-records` (including `getAssignmentStatusSummary`) |
| `src/lib/file-system/__tests__/` | Unit | `file-operations`, `folder-operations`, `format-utils`, `path-utils` |
| `src/app/api/__tests__/` | Integration | `health`, admin user CRUD, `forgot-password`, `reset-password`, document routes (`upload`, `download`, `delete`, `move`, `rename`, `share`, `versions`), admin companies/templates (including comprehension questions)/assignments (including `dueDate`)/completions (including status summary with outstanding users and overdue), customer assignments/completions (including comprehension answer validation and PDF download) |
