...
|---|---|---|
| `src/lib/__tests__/` | Unit | `user-database` (including `jobRole`), `password-reset`, `activity-logger`, `storage`, `url-shortener`, `version-manager`, `list-blobs`, `utils`, `assignments` (including `targetJobRoles` filtering), `completion-records` (including `getAssignmentStatusSummary`) |
| `src/lib/file-system/__tests__/` | Unit | `file-operations`, `folder-operations`, `format-utils`, `path-utils` |
| `src/app/api/__tests__/` | Integration | `health`, admin user CRUD (including `jobRole`), `forgot-password`, `reset-password`, document routes (`upload`, `download`, `delete`, `move`, `rename`, `share`, `versions`), admin companies/templates (including comprehension questions)/assignments (including `dueDate` and `targetJobRoles`)/completions (including status summary with outstanding users and overdue), customer assignments (including `jobRole` filtering)/completions (including comprehension answer validation and PDF download) |
