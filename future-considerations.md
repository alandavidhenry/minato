...
1. ~~SLM-generated comprehension questions with a human-review gate before publish~~ — ✅ Done, admin-portal-only. See "LLM-Generated Comprehension Questions" below.
...
**Known follow-ups:**
- ✅ **Back-link page headers** — `PageHeader` (`src/components/page-header.tsx`) gained optional `backHref`/`backLabel` props (renders a small "← label" link above the title). Rolled out to `admin/companies/[id]`, `admin/completions/[companyId]`, `admin/completions/[companyId]/[templateId]`, and `customer/documents/[assignmentId]/complete`. The remaining non-shell pages (`profile`, `privacy`, `scan`, `documents/error`) now use plain `PageHeader` (no back-link — they aren't nested under a list). `admin/completions/outstanding` and `admin/activity` also moved onto `PageHeader` while making this pass. `signoff/**` kiosk pages were deliberately left on their own larger, high-contrast heading treatment — kiosk legibility from a distance takes priority over chrome consistency there.
- ✅ **`EmptyState`/`TableSkeleton` rollout** — all ~14 bespoke tables (`admin/users`, `admin/templates`, `admin/companies`, `admin/assignments`, `admin/completions` + its `[companyId]`/`[companyId]/[templateId]`/`history`/`outstanding` sub-pages, `admin/activity`, `admin/companies/[id]`'s three tables, `documents/data-table.tsx`, `customer/admin/templates`) plus the card-grid/list pages that used centred paragraphs instead (`customer/documents`, `customer/completions`, `customer/admin/completions`) now use the shared components instead of their own colspan "Loading X..." rows and centred-paragraph empty states.
- ✅ **Dark default on kiosk sign-off** — checked via a headless-browser screenshot of `/signoff/[companyId]` in both dark (default) and forced-light. Text contrast is high in both (near-white on near-black in dark mode comfortably exceeds WCAG AA); no legibility problem found from a contrast standpoint. Note this only verifies contrast in a screenshot — actual glare/reflection on a physical kiosk screen under bright ambient light can't be tested this way and still needs an on-site check if it ever comes up.
- ✅ **Ctrl+K confirmed working** — dispatching a real `KeyboardEvent('keydown', {ctrlKey: true, key: 'k'})` directly against `document` in a headless browser opened the command palette correctly. The earlier automation failure was confirmed to be Playwright/Chromium's synthetic `page.keyboard.press('Control+K')` not reaching the page (an OS/browser-chrome-level interception), not an app bug — `command-palette.tsx`'s listener is not gated by `isAdmin` and works for any authenticated role via the shared `AppShell`.
- ✅ **DataTable/TanStack consolidation** — all ~14 bespoke tables now share one `DataTable` (`src/components/ui/data-table/data-table.tsx`), built on TanStack Table v9's feature API like the documents browser was already using, but with its own leaner feature set (sorting only — no row selection/pagination/filtering) since none of the other pages needed those. Migrated: `admin/companies`, `admin/users` (grouped, one `DataTable` per collapsible role/company section), `admin/templates` (grouped by category), `admin/assignments`, `admin/activity`, `admin/completions` + its `[companyId]`, `[companyId]/[templateId]` (kept its manual `Set`-based row selection for bulk-delete — just rendered through `DataTable`'s columns instead of hand-rolled `<TableRow>`s), `history` and `outstanding` sub-pages, `admin/companies/[id]`'s three tables (company-wide assignments, one per-user assignment group, and the read-only company-created-templates table), `customer/completions`, `customer/admin/completions` (plus the nested completed-records table inside its details dialog) and `customer/admin/templates`. Manual click-to-sort header implementations (`SortArrows` + a `sortKey`/`sortDir` pair, on `admin/assignments` and `admin/completions/outstanding`) were replaced by `DataTable`'s built-in TanStack sorting; `admin/completions/outstanding`'s CSV/XLSX export needed the sort order too, so that page controls `DataTable`'s sort state instead of leaving it uncontrolled. `documents/data-table.tsx` was deliberately left alone — its row-selection/bulk-delete/mobile-card requirements are different enough that sharing `DataTable` would have meant growing it to cover a second, much larger feature surface for one caller. Verified in the browser (dev server + seeded local DB): flat tables, both kinds of collapsible/grouped tables, sortable headers (click-to-sort, arrow indicator swap), and the multi-table company detail page all render and behave correctly; `npm run checks` clean (891 tests).

Key files: `src/app/globals.css`, `src/app/layout.tsx`, `src/components/providers/theme-provider.tsx`, `src/components/app-shell.tsx`, `src/components/app-sidebar.tsx`, `src/components/command-palette.tsx`, `src/components/page-header.tsx`, `src/components/empty-state.tsx`, `src/components/table-skeleton.tsx`, `src/components/ui/data-table/` (data-table, table-features, sort-utils, sort-arrows), `src/components/ui/` (button, badge, card, input, select, textarea, table, tabs, dialog, skeleton, command)
...
## LLM-Generated Comprehension Questions ✅ Done (admin-portal-only)

**Goal:** Reduce Simon's manual effort writing 2–3 comprehension questions per template by letting him click "Generate with AI" and get draft questions suggested from the document's own content, which he reviews/edits before saving — never auto-published.

**Infrastructure:** `infrastructure/modules/ai_foundry/` provisions an Azure AI Foundry (Cognitive Services, `kind = "AIServices"`) account plus a single `GlobalStandard` (pay-per-token, no reserved capacity) model deployment, wired into `modules/minato/main.tf` the same way `gotenberg` is. Model choice: deliberately the cheapest available tier (a "nano"-class GPT model) rather than a dedicated small-language-model family (e.g. Phi) — at this feature's volume (generation only fires on-demand when an admin clicks the button, not per request), the cost difference between an SLM and a cheap LLM is negligible, so reliability of structured JSON output wins. `version_upgrade_option = "NoAutoUpgrade"` is set deliberately so a future model retirement doesn't silently change output quality/cost without a human noticing — check each model's retirement date on its catalog card periodically. **Model name/version in both `terraform.tfvars` files are placeholders pending confirmation against `ai.azure.com/catalog/models`** — the model that prompted this (a GPT tier released after 2026-01) can't be verified against training data.

**Application code (2026-07-31):**
- ✅ `src/lib/storage.ts` — `downloadBlob()` added alongside `uploadBlob`/`deleteBlob`/`generateSasToken`
- ✅ `src/lib/document-text-extraction.ts` — `extractTextFromPdfBuffer()` extracts plain text from an upload template's converted PDF server-side, using `pdfjs-dist`'s **legacy** Node build (already a dependency, previously only used client-side via `react-pdf` in `PDFRenderer.tsx`) — no browser Worker/DOM globals needed. Capped at 20 pages / 15,000 characters to bound token cost regardless of document size.
- ✅ `src/lib/comprehension-question-source.ts` — `getTemplateSourceText()` branches on `sourceType`: for `upload` templates, downloads + extracts the source PDF's text (throws if there's no blob path, or if extraction yields too little text — e.g. a scanned image with no embedded text layer); for `form` templates, concatenates `title` + `description` + non-`section` field labels (no rich body text exists for form templates, so this is thinner source material than an uploaded document gives).
- ✅ `src/lib/comprehension-question-generation.ts` — `generateComprehensionQuestions()` calls the AI Foundry deployment's chat-completions endpoint directly via `fetch` (no SDK dependency, matching `document-conversion.ts`'s Gotenberg-calling convention), requesting `response_format: json_object`. Validates every returned question against the same invariants `edit-template-dialog.tsx`'s client-side `validateForm()` already enforces (≥2 unique non-empty options, answer verbatim-equal to one option) and **filters out** invalid entries rather than failing the whole batch — throws only if zero valid questions remain.
